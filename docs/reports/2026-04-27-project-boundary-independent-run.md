...
- `wechat`、`android`、`ios` 深链目录依赖根目录 `index.html`、`ui/app.js`、`ui/styles.css`，但构建脚本中的路由清单与实际目录清单存在漂移风险。
- 微信小程序实际工程目录是 `wxapp`，而静态预览路由使用 `/wechat/...`，需要在文档中明确映射。
...
2. 微信小程序真实工程在 `wxapp/`，独立 npm 校验。
3. Flutter App 真实工程在 `flutter/`，通过 `ApiClient` 抽象访问后端。
...
|---|---|---|---|
| 根目录 `index.html`/`ui/app.js`/`ui/styles.css` | Cloudflare 文档站与静态 UI 原型 | `npm run build`、`npm run test:product`、`npm run test:routes`、`npm run test:md-preview` | 只读取本地 Markdown 和静态资源，不作为真实客户端运行时代码 |
| `wechat/` | 微信端静态预览深链 | 由根目录构建脚本复制入口并用 Playwright 验证 | 预览路由，不是小程序工程 |
| `wxapp/` | 微信小程序真实工程 | `cd wxapp && npm test` | 通过 API/契约对接服务端；不依赖根目录静态原型运行 |
...
| `flutter/` | Flutter Android/iOS App 工程 | `flutter pub get && flutter test`（当前机器缺 Flutter CLI） | 通过 `ApiClient`/`HttpApiClient` 访问 `/api/v1`，默认可用 mock API 独立预览 UI |
...
- 服务端路由只注册 `/healthz` 与 `/api/v1/...` JSON API，不依赖根目录 `index.html`、`ui/app.js` 或静态文件。
- Flutter 工程通过 `AppConfig` 的 `API_BASE_URL` 和 `USE_MOCK_API` 环境开关区分真实 API 与 mock API；端侧依赖集中在 API 抽象层。
- 微信小程序目录仍命名为 `wxapp/`，静态预览路由命名为 `wechat/`，两者在文档中明确区分。
...
1. 在安装 Flutter SDK 后补跑 `cd flutter && flutter test`，并把 Flutter CI 加入 GitHub Actions。
